...
function mostrarValorAlmacen()
{
    global $inventario;
    echo '<br>';
    $precio = 0; //inicializamos varias variables que nos seran utiles.
    $stock = 0;
    $total = 0;
    foreach ($inventario as $categoria => $comics) { 
        foreach ($comics as $comic) { //entramos a los valores del array
            $precio = $comic['precio']; //cogemos el precio
            $stock = $comic['stock']; //cogemos el stock
            $total += $precio * $stock; //calculamos el total
        }
    }

    echo "Precio en almacen: " . $total . '<br><br>'; //printeamos tras los calculos el precio total
}
...
function aplicarDescuentoManga()
{
    ...
    foreach ($inventario as &$comic) {
        foreach ($comic as &$comics) { //faltaba otro bucle, una vez hecho, funciona a la perfección.
            if ($comics['idioma'] == 'Japonés') {
                $comics['precio'] = $comics['precio'] * 0.7; // Aplicar descuento del 30%
            }
        }
    }
}
